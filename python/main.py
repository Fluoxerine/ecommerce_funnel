...
def main() -> None:
    ...
    # 阶段间共享变量 (初始化为 None, 供后续阶段检查)
    tables = events_cleaned = funnel_wide = None
    page_funnel = strict_funnel = event_funnel = channel_funnels = None
    deep_link_summary = deep_link_ch_comparison = None
    nr_results = weekend_df = None
    channel_df = device_df = country_df = loyalty_df = acq_df = None
    cat_df = duration_df = monthly = cross_df = cohort_retention = None
    churn_features = churn_matrix = None
    loss_df = pie_df = stat_results = ab_groups = camp_roas = None
    ...
    try:
        ...
        logger.info("阶段 1 完成 (%.1fs)", time.time() - t1)
    except Exception:
        logger.error("阶段 1 失败:\n%s", traceback.format_exc())
        raise
    ...
    try:
        ...
        logger.info("阶段 2 完成 (%.1fs), 清洗后 %s 事件, %s 会话",
                    time.time() - t2, f"{len(events_cleaned):,}", f"{len(funnel_wide):,}")
    except Exception:
        logger.error("阶段 2 失败:\n%s", traceback.format_exc())
        raise
    ...
    try:
        ...
        logger.info("阶段 3 完成 (%.1fs)", time.time() - t3)
    except Exception:
        logger.error("阶段 3 失败:\n%s", traceback.format_exc())
        raise
    ...
    try:
        ...
        logger.info("阶段 4 完成 (%.1fs)", time.time() - t4)
    except Exception:
        logger.error("阶段 4 失败:\n%s", traceback.format_exc())
        raise
    ...
    try:
        ...
        logger.info("阶段 5 完成 (%.1fs)", time.time() - t5)
    except Exception:
        logger.error("阶段 5 失败:\n%s", traceback.format_exc())
        raise
    ...
    try:
        ...
        logger.info("阶段 6 完成 (%.1fs)", time.time() - t6)
    except Exception:
        logger.error("阶段 6 失败:\n%s", traceback.format_exc())
        raise
