...
五个渠道中四个的瓶颈在详情页→购物车，但 Direct 渠道在购物车→结算页还有第二道坎。
...
详情页→购物车落在"高影响 + 高易实施"象限（满分 5 分，Impact 4.8 分，Ease 4.2 分），P0 中的 P0。

![PIE矩阵](output/charts/10_pie_matrix.png)

### 新老用户漏斗对比 — 两类用户的行为差异

新用户在首页→列表页流失更严重（缺乏品牌认知），老用户在购物车→结算的转化明显更高。

![新老用户](output/charts/14_new_vs_returning.png)

### 同期群留存热力图 — 用户生命周期视角

每行是一周的新增用户同期群，每列是第 N 周的留存率。首周流失最大（~55%），第 2-4 周趋于平稳。
